...
!
!  Calculate temperature at ice/snow interface (t2)
!    Mellor and Kantha Eq (9a, b)
!
...
!
            qi2(i,j) = alph(i,j)*hicehinv*(ti(i,j,linew)-t2(i,j))
!
...
!    Mellor and Kantha Eq (9b) and following paragraph
!
...
!
!  Snow melting rate equal to snowfall rate (no snow accumulation)
!
                wsm(i,j) = wsf(i,j)
              END IF
            ELSE
...
!
!  There is snow cover to melt, and snow temperature higher than fresh
!    water freezing point (0 degC)
!
                tis(i,j) = 0._r8
!
!  Mellor and Kantha Eq (9b)
...
!    No new snow accumulation in this scenario
!
                wsm(i,j) = wsf(i,j) +                                   &
...
!
      DO j = Jstr,Jend
        DO i = Istr,Iend
!
!  Calculate t0mk, s0mk, qio and wio using three equation formula
!
          tfrz = frln*sice(i,j)
          hfus1(i,j) = hfus*(1.0_r8-brnfr(i,j)) + tfrz*Cp -             &
...
!
            ti(i,j,linew) = 0.5_r8*((qi2(i,j) -  qio(i,j))/cff +        &
     &                              ( t2(i,j) + t0mk(i,j))       )
...
!
          wtot = (1._r8-ai(i,j,linew))*wao(i,j) + ai(i,j,linew)*wio(i,j)
!
